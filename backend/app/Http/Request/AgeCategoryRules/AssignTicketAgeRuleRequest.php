...
class AssignTicketAgeRuleRequest extends BaseRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'rule.bins.*.min' => ['required', 'numeric'],
            'rule.bins.*.max' => ['required', 'numeric'],
            'rule.bins.*.gender' => ['nullable', 'string', 'max:1', 'in:M,F'],
            'rule.bins.*.age_category' => ['required', 'string', 'max:20'],
